...
function zipDirectories( sourceDirs, sourceFiles, outPath ) {
	...
	return new Promise( ( resolve, reject ) => {
		...
		result.append( null, { name: 'wpfeather/' } );

		// Add the directories to the archive.
		sourceDirs.forEach( ( sourceDir ) => {
			result = result.directory( sourceDir, 'wpfeather/' + sourceDir, {
				name: 'wpfeather/' + sourceDir,
			} );
		} );
...
const zipFileName = 'wpfeather.v' + version + '.zip';
zipDirectories(
	[ 'assets/', 'includes/', 'languages/', 'vendor/', 'templates/' ],
	[ 'wpfeather.php', 'readme.txt' ],
	zipFileName
)
	.then( () => {
